...
class Landsat:
    """ Classe modélisant une collection d'images Landsat 8.
        ...
        Attributes:
            ...
            src (str): Source des images ('appeears' ou 'earthdata'). L'intervalle de valeurs valides et les facteurs
    ...
    def getNdvi(self):
        ...
        elif self.src == "earthdata":
            b4_DN = b4_img.getArray(masked=True, lower_valid_range=1, upper_valid_range=65535)
    ...
    def getNdwi(self):
        ...
        if self.src == "appeears":
            ...
            b3_DN = b3_img.getArray(masked=True, lower_valid_range=1, upper_valid_range=65535)
            b3 = np.add(np.multiply(b3_DN, float(0.00002)), float(-0.10))
    ...
    def getAlbedo(self):
        ...
        if self.src == "appeears":
            ...
            b2_DN = b2_img.getArray(masked=True, lower_valid_range=1, upper_valid_range=65535)
            b2 = np.add(np.multiply(b2_DN, float(0.00002)), float(-0.10))
    ...
    def getBand(self, bandNumber):
        ...
        if self.src == "appeears":
            ...
            band_DN = band_img.getArray(masked=True, lower_valid_range=1, upper_valid_range=65535)
            band = np.add(np.multiply(band_DN, float(0.00002)), float(-0.10))
        ...
        """ Permet de reprojeter, découper, aligner et rééchantillonner une image à partir d'une image de référence.
            ...
            appel à la méthode reprojectMatch() de la classe Image.
                Args:
                    ...
                    reduce_zone (bool): Indicateur permettant de choisir si on souhaite réduire la zone d'étude sur
                                        laquelle les images sont "matchées". Ceci est utile pour éviter des problèmes
                                        avec des valeurs nulles sur les bords des images qui s'alignent sur le referenceFile.
                                        Par défaut, cette option est égale à True (donc, on effectue le rétrécissement de zone).
        """
    ...
    def maskClouds30m(self):
        """ Permet de masquer les nuages des images Landsat directement à 30m (avant le rééchantillonnage à 1km).
            Cette méthode utilise la bande de qualité (qa) de Landsat pour effectuer ce masquage en faisant appel à la
            méthode getArray de la classe Image.
        """
        ...
        for band in bandsPaths:
            ...
            band_masked = ma.filled(band_masked, np.nan)  # on remplace les pixels masqués par la valeur np.nan

            # si les images proviennent de earthdata, elles termineront par .TIF et non .tif
            if image.filename.endswith(".TIF"):
    ...
    def maskClouds1000m(self, filename):
        """ Permet de masquer les nuages des images Landsat à 1000m.
            Cette méthode utilise la bande de qualité (qa) de Landsat pour effectuer ce masquage en faisant appel à la
            méthode getArray de la classe Image.
        """
        bandsPaths = [self.b1, self.b2, self.b3, self.b4, self.b5, self.b6, self.b7]
        ...
        for band in bandsPaths:
            ...
            # on masque les pixels de 1000m pour lesquels il y a plus de 10% de pixels de 30m où il y a des nuages
            band_masked = image.getArray(masked=True, lower_valid_range=0, upper_valid_range=10,
                                         cloud_overlay_filename=filename, data_source=self.src)
            band_masked = ma.filled(band_masked, np.nan)  # on remplace les pixels masqués par la valeur np.nan
